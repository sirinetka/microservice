...
public class FactureRestAPI {
	...
	@PutMapping("/Factures/{id}/update-statut")
	public ResponseEntity<Facture> updateFactureStatus (@PathVariable("id") long id,
			String  status){
	...
	public ResponseEntity<Facture> updateFacture (@PathVariable("id") long id,
		...
		if(FacturesInfo.isPresent()){
			...
			System.out.println("update Facture with Id =" + Facture.getPrix() + " .....");
